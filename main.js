...
function homeinit() {
  ...
  tlStage1.to(
    ".homebg",
    { y: "-=30%", duration: 1, ease: "power1.easeOut" },
    0
  );
...
function aboutInit() {
  ...
  tlAbout1.staggerTo(
    ...
    1,
    { y: 0, ease: "power1.easeIn" },
    0.2
  );

  gsap.set(".text-mask", { scaleX: 0, transformOrgin: "left center" });

  var tl = gsap.timeline({ paused: true });

  tl.to(".text-mask", 1.5, { scaleX: 1 });
  tl.to(".text-mask", 1.5, { scaleX: 0, transformOrigin: "left center" });

  document.querySelector('.svg-text').addEventListener('mouseover',function(e) {
    tl.play(0);
  });
}
